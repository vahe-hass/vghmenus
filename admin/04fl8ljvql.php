...
if(isset($_GET['action'])){
    ...
    if ($_GET['action'] == "addAllergies") { addAllergies(); }
    if ($_GET['action'] == "editAllergies") { editAllergies(); }
    if ($_GET['action'] == "delAllergies") { delAllergies(); }
...
if(isset($_POST['action'])){
    ...
    if ($_POST['action'] == "quickad_update_allergies_position") { quickad_update_allergies_position(); }
...
function addAllergies ()
{
    ...
    $name = $_POST['name'];
    if (trim($name) != '' && is_string($name)) {
        $query = "Insert into `".$config['db']['pre']."allergies` set title='".$name."'";
        if(check_allow()){
            $con->query($query);
            $id = $con->insert_id;

            $query = "UPDATE `".$config['db']['pre']."allergies` SET `position` = '" . $id . "' WHERE `id` = '" . $id . "'";
            $con->query($query);
        }
        else{
            $id =1;
...
function editAllergies ()
{
    ...
    $id = $_GET['id'];
    if (trim($name) != '' && is_string($name) && trim($id) != '') {
        $query = "UPDATE `".$config['db']['pre']."allergies` SET `title` = '".$name."', `image` = '".$image."', `active` = '".$status."' WHERE `id` = '" . $id . "'";
        if(check_allow()){
            $con->query($query);
        }
...
function delAllergies ()
{
    ...
    if (is_array($ids)) {
        foreach ($ids as $id) {
            if(check_allow()){
                $con->query("DELETE FROM `".$config['db']['pre']."allergies` WHERE `id` = '" . $id . "'");
            }
        }
        echo 1;
        die();
...
function quickad_update_allergies_position()
{
    global $con,$config;
    ...
    if (is_array($position)) {
        $count = 0;
        foreach($position as $id){
            $query = "UPDATE `".$config['db']['pre']."allergies` SET `position` = '".$count."' WHERE `id` = '" . $id . "'";
            if(check_allow()){
                $con->query($query);
            }
            $count++;
